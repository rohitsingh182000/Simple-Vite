...
function Product() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setData(result?.data?.search?.products);
            console.log(result?.data?.search?.products);
    ...
    return (
        <div>
            ...
            <div >
                <h1 className="text-center font-bold text-5xl mt-5">Products</h1>
                <div className="flex flex-wrap justify-center items-center gap-10 w-full h-full mt-10" >
                    {
                        data?.length == 0 && <p>Loading Data</p>
                    }
                    {data?.map((items: object, index: number) => <Cards key={index} productName={items} />)}
                </div>
            </div>
        </div>
    );
